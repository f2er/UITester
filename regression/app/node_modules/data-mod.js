...
function updateSome(dataArr){
	...
	for(var i =0;i<dataArr.length;i++){
		...
		updateOne('list',item.task_result,nextTime,item.id);
	}
}
...
function _findByTime(){
	var d = new Date();
	var day = d.getDay();
	return "select * from list where locate('"+day+"',week) > 0 and next_time < now()";
}
...
function _updateOneNote(table,result,next_time,id){
   result = JSON.stringify(result).replace(/'/g,"\\'")
    ...
    var sql =  'update '+table+' set task_result = \''+result+'\', next_time = ' + next_time + ', prev_time =now() where id = '+id;
    console.log(sql)
	return sql;
}
