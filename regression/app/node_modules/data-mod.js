...
function _findByTime(){
	var d = new Date();
	var day = d.getDay();
	return "select * from list where locate('"+day+"',week) > 0 and next_time < "+_getDeadline();
}
...
function _updateOneNote(table,result,next_time,id){
   result = result.replace(/"/g,"\\\"")
    ...
    var sql =  'update '+table+' set task_result = "'+result+'", next_time = ' + next_time + ' where id = '+id;
    console.log(sql);
	return sql;
}
...
function _getNewNextTime(duration){
	var cur = new Date();
	var next = cur.valueOf() + duration*6000;
