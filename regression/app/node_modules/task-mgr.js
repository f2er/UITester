...
var TaskManager = {
    ...
    protectTask:function () {
        ...
        setInterval(function () {
            ...
            else if(TaskCache.tasks.length<=10){
                console.log("小于10个，补充新实例")
                TaskManager.fetchTask();
            }
        }, INTERVAL)
    },
    ...
    observeClientAvailable:function () {
        ...
        EventManager.on("client:available", function (client) {
            ...
            console.log(TaskCache.tasks.length+"  tasks-------------")
            for (var i = 0; i < TaskCache.tasks.length; i++) {
                console.log(TaskCache.tasks[i].id, TaskCache.tasks[i]._task_result&&TaskCache.tasks[i]._task_result[type])
            }
            ...
            if (task) {
                ...
                client.run(task,
                    ...
                    function () {
                        if(!isCompelte){
                            ...
                            console.log(task.id + " 超时 in client")
                        }
                ...
                time = setTimeout(function () {
                    if (!isCompelte) {
                        delete    task._task_result[type];
                        console.log(task.id + " 超时 in task")
                    }
